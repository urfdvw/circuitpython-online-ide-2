...
export function createSerialFileSystem(serial) {
    ...
    const rootDirHandle = makeSerialDirectoryHandle({ run, cache, assertCurrent }, "");
    const batch = (fn, opts) => {
        assertCurrent();
        return withSerialSession(serial, async (scopedRun) => {
            const scopedCache = cache.fork(loadTree(scopedRun));
            const root = makeSerialDirectoryHandle({ run: scopedRun, cache: scopedCache, assertCurrent }, "");
            return fn(root);
        }, opts);
    };
