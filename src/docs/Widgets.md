...
- Avoid having both your code and a widget write to the same variable. When a variable is changed from CircuitPython *and* from the widgets, the two writers race each other and whichever wrote last silently wins. The first time the helper sees this happen, it prints a warning to the REPL: `variable '...' is being changed by both CircuitPython and the IDE widgets. To avoid a race condition, consider splitting its role into two separate variables.` As the message suggests, give each side its own variable (for example one to send and one to read back), as in the `brightness` / `br_input` pair above.
