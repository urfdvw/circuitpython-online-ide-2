...
export async function checkEditor() {
    ...
    const retryButton = () => [...document.querySelectorAll("button")].find((element) => element.textContent === "Retry");
    await until(() => window.editorReview && retryButton());
    const state = window.editorReview;
    const first = getEditor(document);
    check("initial editor mounted", !!first);
    check("failed load shows a recoverable error", document.querySelector('[role="alert"]').textContent.includes("Device temporarily unavailable"));
    check("failed load keeps editor read-only", first.getReadOnly());
    first.execCommand("save"); await wait(100);
    check("failed load cannot overwrite the file", state.saved.length === 0);
    retryButton().click();
    await until(() => state.readAttempts === 2 && retryButton());
    check("another failure still offers retry", first.getReadOnly());
    retryButton().click();
    await until(() => state.releaseRead);
    check("pending retry shows loading status", !retryButton() && !!document.querySelector('[role="status"]'));
    first.execCommand("save"); await wait(100);
    check("pending retry still prevents writes", first.getReadOnly() && state.saved.length === 0);
    state.releaseRead();
    await until(() => first.getValue().includes("original") && !first.getReadOnly());
    check("successful retry restores editing in the same tab", getEditor(document) === first);
    check("successful retry clears the load error", !document.querySelector('[role="alert"]'));
